...
const examInput = z.object({ title: z.string().trim().min(2).max(160), subject: z.string().trim().max(120), className: z.string().trim().max(120), examDate: z.string().optional(), questionCount: z.coerce.number().int().min(1).max(120), alternativeCount: z.coerce.number().int().min(2).max(6) });
async function teacherId() { const id = await currentUserId(); if (!id) redirect("/auth"); return id; }

export async function createExam(formData: FormData) {
  const input = examInput.parse(Object.fromEntries(formData)); const id = await teacherId(); await connectDatabase();
  const exam = await ExamModel.create({ ...input, teacherId: id, examDate: input.examDate ? new Date(input.examDate) : null });
